...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/contacts' && request.method === 'POST') {
      ...
      let body;
      try { body = await request.json(); } catch { return new Response('Bad request', { status: 400 }); }
      const { name='', org='', title='', phone='', email='', website='', street='', postal='', city='', country='', replaceId=null } = body;
      if (replaceId) {
        await env.DB.prepare('DELETE FROM contacts WHERE id = ?').bind(replaceId).run();
      }
      const insert = () => env.DB.prepare(
        'INSERT INTO contacts (name,org,title,phone,email,website,street,postal,city,country,scanned_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)'
      ).bind(name, org, title, phone, email, website, street, postal, city, country, new Date().toISOString()).run();
      try {
        await insert();
      } catch (e) {
        // Bestaande database mist een van de adreskolommen → eenmalig toevoegen en opnieuw proberen
        for (const col of ['street', 'postal', 'city', 'country']) {
          try { await env.DB.prepare(`ALTER TABLE contacts ADD COLUMN ${col} TEXT`).run(); } catch {}
        }
        await insert();
      }
      return new Response(JSON.stringify({ ok: true }), {
...
<table>
  <thead><tr><th>Naam</th><th>Bedrijf</th><th>Functie</th><th>Telefoon</th><th>E-mail</th><th>Website</th><th>Straat</th><th>Postcode</th><th>Plaats</th><th>Land</th><th>Datum</th></tr></thead>
